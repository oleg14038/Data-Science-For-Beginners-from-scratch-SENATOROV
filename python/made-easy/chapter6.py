...
# round(m.пaЬer [ , ndigits] ) округляет число до указанного количества знаков
#
...
def parrot(
    ...
    """Функция выводит информацию о попугае в зависимости от состояния и
    напряжения.
...
# 5. Три позиционных аргумента (обратите внимание, что функции паррот не хватает
# кода, чтобы обработать больше трех параметров. Предположим, что паррот
...
# parrot()  # это вызовет ошибку TypeError, потому что `volt` обязательный
...
# parrot(110, volt=220)  # Это вызовет ошибку, потому что аргумент `volt` передается дважды

# 10. Ошибка: неизвестный именованный аргумент
# parrot(act="Jgon KLiz")  # Это вызовет ошибку, если `act` не является ожидаемым аргументом.
# Это не ошибка синтаксиса, но логическая ошибка, если аргумент `act` не предусмотрен.


# +
def try_function(*args: str, **kwargs: str | int) -> None:
    """Функция принимает произвольное количество позиционных и именованных
    аргументов.

    :param args: Позиционные аргументы, каждый из которых является строкой.
    :param kwargs: Именованные аргументы, где ключи - строки, а значения могут быть строками или числами.
    """
...
# 6.6. Генераторы
# Генератор - это функция, возвращающая итератор. То есть эти функции генерируют объект,
# который можно перебирать в цикле. Генератор - это особый класс функций. Вместо того,
# чтобы возвращать одно значение, функция-генератор возвращает последовательность
# значений (объект-итератор). Слово return в таких функциях заменяется на yield.


def inclusive_range(*args: int) -> Iterator[int]:
    """Функция, которая генерирует последовательность чисел от start до stop с
    шагом step.
...
# В Python items() — это метод, который используется для получения представления
# элементов словаря. Он возвращает представление, состоящее из пар ключ-значение,
