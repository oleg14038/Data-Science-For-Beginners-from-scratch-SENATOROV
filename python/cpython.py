...
#     Компиляции в байт-код, который выполняется быстрее, чем чистый исходны
# й код.
...
#
# В папке include CPython находятся заголовочные файлы (.h), необходимые 
# для компиляции и взаимодействия с ядром Python.
#
...
#     Из токенов строится абстрактное синтаксическое дерево (AST), отражающее 
# структуру программы.
...
# (Just-In-Time), 
# которая позволяет значительно ускорять выполнение кода, тогда как CPython 
# интерпретирует байт-код построчно. Это делает PyPy быстрее в большинстве
# случаев, 
# особенно в вычислительно сложных задачах.
#
# 14.2 CPython поддерживает все C-расширения, такие как NumPy и TensorFlow, 
# без ограничений, тогда как PyPy может испытывать сложности с
# некоторыми из них.
#
...
# эффективный механизм сборки мусора без счетчика ссылок, что снижает 
# нагрузку на систему.
#
# 14.4 PyPy также более экономно использует память по сравнению с CPython,
# но из-за особенностей JIT-компиляции он может потреблять
# больше ресурсов в начале работы программы.
#
...
# зависят от модулей, написанных на C. PyPy поддерживает не все
# C-расширения или может работать с ними медленнее, чем CPython.
#
...
# может происходить дополнительная нагрузка на процессор. Это делает PyPy менее
# эффективным для небольших скриптов и командных утилит.
#
#     Не всегда 100% совместимость с CPython
#     Хотя PyPy старается поддерживать стандарт Python, его внутренняя 
# реализация может вести себя немного иначе, что иногда вызывает 
# несовместимость с кодом, написанным для CPython.
#
#     Ограниченная поддержка от разработчиков библиотек
#     Большинство библиотек разрабатываются и тестируются с учетом CPython, а 
# PyPy рассматривается как второстепенная платформа, что может привести к 
# неожиданным проблемам.
#
# Из-за этих ограничений PyPy лучше всего подходит для вычислительных задач,
# но не всегда удобен для проектов, активно использующих C-расширения и 
# быстрый запуск.
...
# Если вы не добавили PyPy в переменную PATH, укажите полный путь
# к исполняемому файлу pypy:
#
...
# горячие участки кода в машинный код, что снижает накладные расходы
# на интерпретацию и повышает общую скорость выполнения программ.
#
# Однако стоит отметить, что выигрыш в производительности может варьироваться 
# в зависимости от конкретного приложения и его характера. В некоторых случаях, 
# особенно при использовании C-расширений или ввода-вывода, разница
# в скорости может быть менее заметной.
