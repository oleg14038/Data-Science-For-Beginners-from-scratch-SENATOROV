...
#    В Jupyter Notebook номера ячеек (In [1]:, In [2]: и т. д.) обновляются при выполнении ячеек.

#    Если выполнить ячейку повторно, её номер увеличится.

# VS Code (Jupyter Kernel)

#    В VS Code при использовании Jupyter ядра номера ячеек тоже изменяются динамически.

#    Если перезапустить ядро, нумерация начинается заново.

# Терминал (IPython)

#    В IPython номера команд (In [1]:, In [2]:) идут последовательно и продолжаются даже после выполнения новых команд.
